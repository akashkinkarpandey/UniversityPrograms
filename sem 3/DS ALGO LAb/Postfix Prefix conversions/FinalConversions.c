//All types of conversions
#include <stdio.h>
char a[100];
char b[100];
int i = -1;
//LIFO or FILO principle for stacks
void push1(int x)
{
    if (i == 99) //max size reached
    {
        printf("Overflow\n");
        return;
    }
    a[++i] = x; //insert
}
int isEmpty1()
{
    return (i == -1); //if no element has been inserted yet
}
int size1()
{
    return (i + 1);
}
void pop1()
{
    if (isEmpty1()) //underflow checking
    {
        // printf("Nothing to be deleted\n");
        return;
    }
    // printf("%d deleted\n", a[i]);
    i--; //reducing index
}
int top1()
{
    if (isEmpty1())
        return -99;
    return a[i];
}

void print1()
{
    if (isEmpty1()) //underflow checking
    {
        printf("Nothing to be printed\n");
        return;
    }
    for (int j = i; j >= 0; j--) //topmost elements printed first
        printf("%d ", a[j]);
    printf("\n");
}

int isOperator(char c)
{
    if (c == '+' || c == '-' || c == '*' || c == '/')
        return 1;
    return 0;
}
int isNumeric(char c)
{
    if (c >= '0' && c <= '9')
        return 1;
    return 0;
}
//1 prefix evaluation(prefix to infix)
void PreFixEvaluation()
{
    printf("Enter a correct PreFix Expression\n");
    // gets(a);
    scanf(" %[^\n]%*c", a);
    int res;
    int c = 0;
    while (a[c++] != '\0');
    int len = c - 1;
    int i = 0, j = len - 1;
    while (i <= j)
    {
        int t = a[i];
        a[i] = a[j];
        a[j] = t;
        i++;
        j--;
    }
    // puts(a);
    for (i = 0; i < len; i++)
    {
        // printf("%d %d %c\n",i,a[i],a[i]);
        if (a[i] == ' ')
            continue;
        if (isNumeric(a[i]))
            push1(a[i] - '0');
        else if (isOperator(a[i]))
        {
            int f1 = top1();
            pop1();
            if (f1 == -99)
            {
                printf("Wrong expression exiting\n");
                return;
            }
            int f2 = top1();
            pop1();
            if (f2 == -99)
            {
                printf("Wrong expression exiting\n");
                return;
            }
            if (a[i] == '+')
                res = f1 + f2;
            else if (a[i] == '-')
                res = f1 - f2;
            else if (a[i] == '*')
                res = f1 * f2;
            else if (a[i] == '/')
                res = f1 / f2;
            else
            {
                printf("Wrong expression  exiting\n");
                return;
            }
            push1(res);
        }
        else
        {
            printf("Wrong expression  exiting\n");
            return;
        }
    }
    printf("result is %d\n", res);
}
//2 postfix evaluation(postfix to infix)
void PostFixEvaluation()
{
    printf("Enter a correct Postfix Expression\n");
    // gets(a);
    scanf(" %[^\n]%*c", a);

    int res;
    int c = 0;
    while (a[c++] != '\0')
        ;
    int len = c - 1;
    for (int i = 0; i < len; i++)
    {
        if (a[i] == ' ')
            continue;
        if (isNumeric(a[i]))
            push1(a[i] - '0');
        else if (isOperator(a[i]))
        {
            int f1 = top1();
            pop1();
            if (f1 == -99)
            {
                printf("Wrong expression exiting\n");
                return;
            }
            int f2 = top1();
            pop1();
            if (f2 == -99)
            {
                printf("Wrong expression exiting\n");
                return;
            }
            if (a[i] == '+')
                res = f1 + f2;
            else if (a[i] == '-')
                res = f2 - f1;
            else if (a[i] == '*')
                res = f1 * f2;
            else if (a[i] == '/')
                res = f2 / f1;
            else
            {
                printf("Wrong expression exiting\n");
                return;
            }
            push1(res);
        }
        else
        {
            printf("Wrong expression exiting\n");
            return;
        }
    }
    printf("%d\n", res);
}
// 3 infix to prefix
int prec(char s)
{
    if (s == '^')
        return 3;
    else if (s == '*' || s == '/')
        return 2;
    else if (s == '+' || s == '-')
        return 1;
}
void InfixtoPrefix()
{
    printf("Enter a correct Infix Expression\n");
    // gets(a);
    scanf(" %[^\n]%*c", a);

    int c = 0, f = 0;
    while (a[c++] != '\0')
        ;
    int len = c - 1;
    int i = 0, j = len - 1;
    while (i <= j)
    {
        int t = a[i];
        a[i] = a[j];
        a[j] = t;
        i++;
        j--;
    }
    puts(a);
    for (i = 0; i < len; i++)
    {
        if (a[i] == '(')
        {
            a[i] = ')';
            continue;
        }
        if (a[i] == ')')
            a[i] = '(';
    }
    // puts(a);
    for (i = 0; i < len; i++)
    {
        char t = a[i];
        if (t == ' ')
            continue;
        if ((t >= 'a' && t <= 'z') || (t >= 'A' && t <= 'Z'))
            b[f++] = t;
        else if (t == '(')
        {
            push1(t);
            b[f++] = t;
        }
        else if (t == ')')
        {
            while (isEmpty1() == 0 && top1() != '(')
            {
                b[f++] = top1();
                pop1();
            }
            if (isEmpty1() == 0)
            {
                // b[f++]=top1();
                pop1();
            }
            b[f++] = t;
        }
        else
        {
            while (isEmpty1() == 0 && prec(top1()) >= prec(t) && top1() != '(')
            {
                b[f++] = top1();
                pop1();
            }
            push1(t);
        }
    }
    while (isEmpty1() == 0)
    {
        b[f++] = top1();
        pop1();
    }
    i = 0, j = len - 1;
    while (i <= j)
    {
        int t = b[i];
        b[i] = b[j];
        b[j] = t;
        i++;
        j--;
    }
    for (i = 0; i < len; i++)
    {
        if (b[i] == '(')
        {
            b[i] = ')';
            continue;
        }
        if (b[i] == ')')
            b[i] = '(';
    }
    puts(b);
}
//4 infix to postfix
void InfixtoPostfix()
{
    printf("Enter a correct Infix Expression\n");
    // gets(a);
    scanf(" %[^\n]%*c", a);
    int c = 0, f = 0;
    while (a[c++] != '\0');
    int len = c - 1;
    for (int i = 0; i < len; i++)
    {
        char t = a[i];
        if(t==' ')
        continue;
        if ((t >= 'a' && t <= 'z') || (t >= 'A' && t <= 'Z'))
            b[f++] = t;
        else if (t == '(')
        {   
            push1(t);
            b[f++] = t;
        }
        else if (t == ')')
        {
            while (isEmpty1() == 0 && top1() != '(')
            {
                b[f++] = top1();
                pop1();
            }
            if (isEmpty1() == 0)
            {   
                // b[f++]=top1();
                pop1();
            }
            b[f++]=t;
        }
        else
        {
            while (isEmpty1() == 0 && prec(top1()) >= prec(t) && top1()!='(')
            {
                b[f++] = top1();
                pop1();
            }
            push1(t);
        }
    }
    while (isEmpty1() == 0)
    {
        b[f++] = top1();
        pop1();
    }
    puts(b);
}
int main()
{
    int choice;
    while (1)
    {
        printf("Enter 1 for conversion from prefix to infix\n");
        printf("Enter 2 for conversion from postfix to infix\n");
        printf("Enter 3 for conversion from infix to postfix\n");
        printf("Enter 4 for conversion from infix to prefix\n");
        printf("Enter any other number for exiting\n");
        scanf("%d", &choice);
        switch (choice)
        {
        case 1:
            printf("No characters invloved,only numbers\n");
            PreFixEvaluation();
            break;
        case 2:
            printf("No characters invloved,only numbers\n");
            PostFixEvaluation();
            break;
        case 3:
            InfixtoPostfix();
            break;
        case 4:
            InfixtoPrefix();
            break;
        default:
            printf("Exiting\n");
            return 0;
        }
    }
}